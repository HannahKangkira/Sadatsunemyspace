...
def callback_api_query_roadname() -> dict:
    println(f"{request.remote_addr} {request.method} {request.path}")
    ...
    try:
        with connection.cursor() as cursor:
            ...
            random_number = random.randrange(1, 425) 
            sql_query = "SELECT * FROM roadname where FID = "+ str(random_number)
            cursor.execute(sql_query)
            result = cursor.fetchall()
        ...
        # 关闭数据库连接
        connection.close()
    return generate_response(200, False, "roadname fetched!", result_list)

def callback_api_query_getstamps() -> dict:
    println(f"{request.remote_addr} {request.method} {request.path}")
    ...
    try:
        with connection.cursor() as cursor:
            sql_query = "SELECT * FROM stamps WHERE FID BETWEEN %s AND %s"
        ...
        # 关闭数据库连接
        connection.close()
    return generate_response(200, False, "stamps fetched!", result_list)

def callback_api_query_getqsls() -> dict:
    println(f"{request.remote_addr} {request.method} {request.path}")
    start = request.form.get("start")
    end = request.form.get("end")
    print(f"Query from {start} to {end}")
    ...
    try:
        with connection.cursor() as cursor:
            sql_query = "SELECT * FROM qsl WHERE FID BETWEEN %s AND %s"
            cursor.execute(sql_query, (start, end))
            result = cursor.fetchall()
            ...
            for row in result:
                row_dict = {
                    "FID": row[0],
                    "name": row[1],
                    "dataside": base64.b64encode(row[2]).decode('utf-8') if row[2] else None,
                    "backside": base64.b64encode(row[3]).decode('utf-8') if row[3] else None,
                }
                result_list.append(row_dict)
        ...
        # 关闭数据库连接
        connection.close()
    return generate_response(200, False, "QSL pics fetched!", result_list)
